 ...
 */
int write_to_usb(char *data, int data_len){
    int fd = open_usb_fd();
    ssize_t num_written;
 ...
 * of a physical symbols matrix. The function "set_size" takes this parameter and checks if it is equal to "a ", "b ",
 * or "c ". If it is, it calls the function "write_to_usb
 * 
 * @return The function `set_size` returns an integer value. If the `size` parameter is equal to "a ",
 * "b ", or "c ", it calls the `write_to_usb` function with the `size` parameter and returns the value
...
int set_size(char* size){
    if (strcmp(size, "a ") == 0 || strcmp(size, "b ") == 0 || strcmp(size, "c ") == 0) {
